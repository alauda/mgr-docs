...
4. **Migrate Users and Privileges**:
   ...
   # For each user, export grants from source and apply on target
   for user_host in ${USERS}; do
     echo "Migrating user: ${user_host}..."

     # Get CREATE USER and GRANT statements from source
     ...
     # Ensure mysql_native_password for compatibility with older clients
     kubectl exec ${TARGET_NAME}-0 -n ${TARGET_NAMESPACE} -c mysql -- \
       mysql -uroot -p${TARGET_MYSQL_PASSWORD} -e "
         ALTER USER ${user_host} IDENTIFIED WITH mysql_native_password BY 'CHANGE_ME';
       "
   done
   ...
   :::warning
   The `ALTER USER ... BY 'CHANGE_ME'` resets passwords. Replace `'CHANGE_ME'` with each user's actual password, or have users reset their passwords after migration.
   :::
