...
## Installing Alauda Data Services Relational Database E1

### Prerequisites

1. Download the plugin installation package that matches the platform.
2. Use the platform's application publishing capability to publish the plugin to any cluster where Alauda Data Services Relational Database E1 capabilities are desired.

::: warning Processor requirement
The 4.4 line requires cluster nodes with a processor supporting the **x86-64-v3** microarchitecture level (Intel
Haswell, AMD Excavator, or later). On older processors the operator and the MySQL server, router, and client
containers abort at start with `Fatal glibc error: CPU does not support x86-64-v3`.

This requirement comes from the AlmaLinux 10 base image these components are built on and applies to the whole 4.4
line. The monitoring exporter is not affected. On hardware that does not meet this requirement, use the 4.3 line.
:::
...
<Tabs>
  <Tab label="Web Console">
    1. Log in to the platform and navigate to the **Administrator** page.
    2. In the left navigation bar, select **Marketplace** -> **OperatorHub** to enter the **OperatorHub** page.
    3. Find **Alauda Data Services Relational Database E1**, click **Install** to access the installation page.
