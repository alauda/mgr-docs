...
- Fixed an issue where creating a MySQL instance failed with `ImagePullBackOff` on the agent sidecar, even though the
  ...
  v4.4.1 is affected**; upgrade to v4.4.3 (or v4.3.5 on the 4.3 line) to resolve it. Instances already running are not
  affected.
...
<!-- TODO(v4.4.3): fill from the wave A/B scan results before opening the PR. Cover: AlmaLinux 10 base refresh for the
