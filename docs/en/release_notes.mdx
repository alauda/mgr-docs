...
- Fixed an issue where creating a MySQL instance failed with `ImagePullBackOff` on the agent sidecar, even though the
  ...
  v4.4.1 is affected**; upgrade to v4.4.2 (or v4.3.5 on the 4.3 line) to resolve it. Instances already running are not
  affected.
...
<!-- TODO(v4.4.2): fill from the wave A/B scan results before opening the PR. Cover: AlmaLinux 10 base refresh for the
     MySQL server/router/client images (openssl-libs, libevent), the mysqld-exporter golang.org/x/crypto update, and
     the slow-query agent gRPC update. State the resulting HIGH/CRITICAL count. -->
